    ...
    <Link :href="route('movies.show', { movie: banner.slug })" class="xl:min-w-[70%] lg:min-w-3/5 w-full">
      <ZoomableBanner :image="banner.image">
        <div class="self-end text-white lg:p-10 md:p-8 p-6">
          <h2 class="lg:text-5xl md:text-4xl text-3xl font-semibold">
            {{ banner.title }}
          </h2>
    ...
    <div class="border border-neutral-800 xl:w-full lg:w-1/2 min-h-full rounded-3xl hidden lg:block">
      <div class="bg-tertiary text-white w-full min-h-full rounded-3xl p-6 flex justify-between flex-col">
        <ul class="space-y-[5px]">
          ...
          <NoteItem title="Жанр:">
            {{ formatGenres(banner.genres) }}
          </NoteItem>
          <NoteItem title="В головних ролях:">
            {{ banner.main_cast }}
          </NoteItem>
          <NoteItem title="Країна:">
            {{ banner.production_country }}
          </NoteItem>
          <NoteItem title="Цікавий факт:">
            {{ banner.fact }}
          </NoteItem>
        </ul>
        <Link
          :href="route('movies.show', { movie: banner.slug })"
