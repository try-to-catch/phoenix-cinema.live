...
const { seatingPlan, screening } = defineProps<{
...
const extendedSelectedSeats = computed(() => {
  return orderForm.seat_ids.map(seatId => {
    const seat = seatingPlan.flat().find(seat => seat.id === seatId) as ISeat
...
type AuthSuggestionModalInstance = InstanceType<typeof AuthSuggestionModal>
// eslint-disable-next-line @typescript-eslint/no-redundant-type-constituents,@typescript-eslint/no-unsafe-assignment
const authSuggestionModalRef = ref<AuthSuggestionModalInstance | null>(null)
const suggestAuth = () => {
    ...
    <section class="pt-5 sm:pt-1">
      <div class="container">
        <div class="xl:w-4/5 2xl:w-3/4 xl:mx-auto">
          <h3 class="sm:hidden font-medium text-lg text-white mb-1.5">
            {{ screening.movie.title }}
          </h3>

          <div class="flex w-full">
            <MoviePoster
              :age-restriction="screening.movie.age_restriction"
            ...
            <div class="ml-4 w-full space-y-2 sm:space-y-3 md:space-y-4">
              ...
              <div class="flex lg:flex-row flex-col lg:space-x-2">
                <InfoItem
                  :bottom-text="screening.hall.address"
                ...
                <InfoItem :bottom-text="screening.start_date_day_of_weak" :top-text="screening.start_date">
                  <CalendarDays class="lg:w-6 lg:h-6 md:w-5 md:h-5 w-4 h-4 fill-white" />
                </InfoItem>

                <InfoItem :bottom-text="`${screening.start_time} - ${screening.end_time}`" top-text="Час">
        ...
        <div class="xl:w-4/5 2xl:w-3/4 xl:mx-auto">
          <div class="flex flex-col justify-center">
            <div class="flex space-x-4 justify-center">
              <SeatTypePrice
                :label="SeatType.Standard"
                :price="screening.standard_seat_price"
                seat-color-class="bg-blue-400"
              />

              <SeatTypePrice
                :label="SeatType.Premium"
                :price="screening.premium_seat_price"
